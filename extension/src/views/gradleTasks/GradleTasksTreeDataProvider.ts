...
export class GradleTasksTreeDataProvider
  ...
  public async getChildren(
    ...
    if (element instanceof ProjectTreeItem) {
      ...
      const dependencyItem = await this.client.getDependencies(path.dirname(resourceUri.fsPath), getGradleConfig());
      if (!dependencyItem) {
        return results;
      }
      const projectDependencyItem = protocolItem2ProjectDependencyTreeItem(dependencyItem, element);
