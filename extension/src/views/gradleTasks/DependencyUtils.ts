...
export function protocolItem2ProjectDependencyTreeItem(protocolItem: DependencyItem, parent: vscode.TreeItem): ProjectDependencyTreeItem | undefined {
  const name = "Dependencies";
  const projectItem: ProjectDependencyTreeItem = new ProjectDependencyTreeItem(name, vscode.TreeItemCollapsibleState.Collapsed, parent);
  ...
  for (const child of children) {
    ...
    const configurationItem = protocolItem2DependencyConfigurationTreeItem(child, projectItem);
...
export function protocolItem2DependencyConfigurationTreeItem(protocolItem: DependencyItem, parent: vscode.TreeItem): DependencyConfigurationTreeItem | undefined {
  const name = protocolItem.getName();
  const storageMap = new Map();
  const configurationItem: DependencyConfigurationTreeItem = new DependencyConfigurationTreeItem(name, vscode.TreeItemCollapsibleState.Collapsed, parent);
  const children = protocolItem.getChildrenList();
  const treeChildren = [];
  for (const child of children) {
...
export function protocolItem2DependencyTreeItem(protocolItem: DependencyItem, parent: vscode.TreeItem, storageMap: Map<string, vscode.TreeItem>): DependencyTreeItem {
  const name = protocolItem.getName();
  const dependencyItem: DependencyTreeItem = new DependencyTreeItem(name, vscode.TreeItemCollapsibleState.Collapsed, parent);
